...
function handleSendMessage(message) {
  ...
  encodedMessage = message
    ...
    : encodeURIComponent(messageDefault);
...
function messageGeneratorOrder(product) {
  ...
  message +=
    "Para confirmar seu pedido, por favor, responda com *'Confirmo'* ou envie uma mensagem para mais informações.";
...
function initRenderProducts() {
  products.map((item, index) => {
                ...
                <li class="products_content-item" id="prodcut-item-${index}">
                    ...
                    <div class="product-item-body">
                        ...
                        ${
                          item.ESGOTADO
                            ? `
                            <button disabled id="button-item-${index}" class="button-disabled" style="background-color: grey !important;">
                                Esgotado <img src="./assets/icons/logo-whatsapp-green.svg">
                            </button>`
                            : `
    ...
    if (sectionProductContet) {
      ...
      document
        .getElementById(`button-item-${index}`)
...
wppUtilityLink.forEach((link) => {
  link.addEventListener("click", () =>
    handleSendMessage("Olá, gostaria de fazer um pedido!")
  );
});
